...
**The RBAC seam is `visibleServers(userId)`** in `server/src/registry/index.ts`: it ignores its
argument and returns all enabled servers (v1 open policy). The old `user_server_access` join table
(never read) was removed; to implement RBAC, add it back and consult it here, since every routing and
listing path already goes through this seam. Constraint: "not permitted" must be **indistinguishable**
from "does not exist", or the API becomes an enumeration oracle.
